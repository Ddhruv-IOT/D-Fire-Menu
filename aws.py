...
def aws():
    """
    AWS Menu: This function provides a menu-driven interface to interact with various AWS functionalities.

    The user can choose from the following options:
            1. To configure AWS
            2. To create a key-pair
            3. To create a security-group
            4. To launch an instance
            5. To start an instance
            6. To stop an instance
            7. To create an EBS volume of 1 GB
            8. To create an S3 bucket
            9. To upload an image in the bucket
            10. To create a Cloudfront Distribution
            11. To create a Snapshot
            12. To go back to the main menu
            13. To close the program
    ...
    while True:
        ...
        if int(ch) == 1:
            ...
            os.system(
                "aws ec2 create-security-group --description {} --group-name {}".format(
            ...
            keyname = input("\nEnter the keyname :-")
            os.system(
                "aws ec2 run-instances --image-id {} --instance-type t2.micro --security-group-ids {} --key-name {}".format(
                    imageid, securityid, keyname
                )
            )
            ...
            if int(s) == 1:
                ...
                az = "ap-south-1c"
                os.system(
                    "aws ec2 create-volume --volume-type gp2 --size 1 --availability-zone ap-south-1c"
                )
            ...
            region = input("\nEnter the region :-")
            os.system(
                "aws s3api create-bucket --bucket {} --region {} --create-bucket-configuration LocationConstraint=ap-south-1".format(
                    bname, region
                )
            )
            ...
            iname = input("\nEnter image name :-")
            os.system(
                "aws s3api put-object --bucket {} --body {} --key {}".format(
                    bname, location, iname
                )
            )
            ...
            os.system(
                "aws ec2 create-snapshot --volume-id {} --description {}".format(
                    vd, desc
                )
            )
